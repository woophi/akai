...
export const saveBiography = async (req: Request, res: Response, next: NextFunction) => {
  const validate = new kia.Validator(req, res, next);

  const biographyData: models.BiographySaveModel = {
    bio: req.body.bio,
    coverPhoto: req.body.photoId,
    createdBy: (req.session as unknown as SessionData).userId ?? '',
  };
  async.series(
    [
      cb =>
...
export const getAdminBiography = async (req: Request, res: Response, next: NextFunction) => {
