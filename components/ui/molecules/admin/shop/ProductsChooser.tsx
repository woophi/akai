...
export const ProductsChooser = React.memo<Props>(
  ({
    label = 'Добавить продукт',
    error,
    disabled = false,
    onConfirm,
    subLabel = 'Продукты в категории',
    single = false,
  }) => {
    ...
    return (
      <Box margin="0 1rem 1rem">
        <InputLabel style={{ color: '#000', marginBottom: '.5rem' }}>{subLabel}</InputLabel>
        ...
        <ModalDialog open={open} onClose={handleClickClose} onConfirm={handleConfirm} title={'Выберите продукты'}>
          <ProductsList onClickCb={chooseProduct} selectedProducts={chosenProducts} single={single} />
        </ModalDialog>
      </Box>
    );
  }
