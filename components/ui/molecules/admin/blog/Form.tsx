...
export const BlogForm = React.memo<Props>(({ blogId, initialValues = {}, albums }) => {
  ...
  const hundleDeletBlog = () => {
    if (blogId) return deleteBlog(blogId);
      ...
      <Form
        ...
        initialValues={
          blogId
            ? {
                ...initialValues,
                creationPictureDate: moment((initialValues as BlogData).creationPictureDate).format('YYYY-MM-DD'),
              }
            : {
            ...
            {!blogId && (
              <>
                <Field
                  ...
                  render={({ input }) => (
                    <FormControlLabel
                      control={<Switch checked={input.value} onChange={input.onChange} value="checkedB" color="primary" />}
                ...
                <Field
                  ...
                  render={({ input, meta }) => (
                    <FormControl variant="outlined" className={classes.field}>
                      <InputLabel htmlFor="localeId" ref={inputLabelSLID} style={{ color: theme.palette.primary.main }}>
                ...
                <Field
                  ...
                  render={({ input, meta }) => (
                    <FormControl variant="outlined" className={classes.field}>
                      <InputLabel htmlFor="photo" ref={inputLabelSP} style={{ color: theme.palette.primary.main }}>
                        {'Выбрать картину для публикации на facebook и instagram'}
                      </InputLabel>
                      <Select
                        {...input}
                        input={<OutlinedInput labelWidth={labelWidthSP} id="photo" />}
                        error={Boolean(meta.touched && meta.error)}
                        disabled={submitting || !getPhotoValues(form.getState().values as BlogData).length}
                      >
                ...
                <Field
                  ...
                  render={({ input, meta }) => (
                    <FormControl variant="outlined" className={classes.field}>
                      <InputLabel htmlFor="albumId" ref={inputLabelAlbum} style={{ color: theme.palette.primary.main }}>
                        {'Назначить в альбом'}
                      </InputLabel>
                      <Select
                        {...input}
                        input={<OutlinedInput labelWidth={labelWidthAlbum} id="albumId" />}
                        error={Boolean(meta.touched && meta.error)}
            ...
            <FieldArray name="parameters">
              {({ fields }) => (
                <div className={classes.field}>
                  <InputLabel style={{ color: '#000', marginBottom: '.5rem' }}>{'Параметры картины'}</InputLabel>
