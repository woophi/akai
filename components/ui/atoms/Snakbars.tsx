...
export const Snakbars = React.memo<Props>(props => {
  ...
  const calcValue = Number(((timer / (timerValue || INITIAL_TIMER_VALUE)) * 100).toFixed(0));
    ...
    >
      <Alert severity={variant} className={classes.alertCont}>
        <div className={classes.container}>
          ...
          <IconButton key="close" aria-label="Close" color="inherit" onClick={handleClose} className={classes.iconButton}>
            <CloseIcon className={classes.icon} />
          </IconButton>
        </div>
        <Box position="absolute" bottom="0" left="0" width="100%">
          <LinearProgress variant="determinate" value={calcValue} color="secondary" className={classes[variant]} />
        </Box>
      </Alert>
    </Snackbar>
  );
